  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        ...
        style={{ maxWidth: 520, maxHeight: '92vh', background: '#FFFFFF', borderRadius: 16, boxShadow: '0px 20px 50px rgba(0,0,0,0.2)' }}
      >
        {/* Header */}
        <div className="flex-shrink-0 flex items-start justify-between px-8 pt-8 pb-6" style={{ borderBottom: '1px solid #F1F5F9' }}>
          <div>
            ...
            <p className="text-sm mt-1" style={{ color: '#64748B' }}>Enter details for the new gallery item.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-8 py-6 space-y-5">
          ...
          <div className="space-y-2">
            <label className="block text-sm font-semibold" style={{ color: '#1A1C1F' }}>Product Name</label>
            <input type="text" name="name" value={form.name} onChange={handleChange} placeholder="e.g. Vintage Walnut Frame" style={inputStyle} />
          </div>

          {/* ID + Category */}
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="block text-sm font-semibold" style={{ color: '#1A1C1F' }}>Product ID</label>
              <input type="text" name="productId" value={form.productId} onChange={handleChange} placeholder="SKU-000" style={inputStyle} />
            </div>
            <div className="space-y-2">
              <label className="block text-sm font-semibold" style={{ color: '#1A1C1F' }}>Category</label>
              <div className="relative">
                <select name="category" value={form.category} onChange={handleChange} className="appearance-none cursor-pointer" style={inputStyle}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="block text-sm font-semibold" style={{ color: '#1A1C1F' }}>Stock</label>
          ...
          {/* Description */}
          <div className="space-y-2">
            <label className="block text-sm font-semibold" style={{ color: '#1A1C1F' }}>Product Description</label>
            <textarea
              name="description"
              ...
              style={{ background: '#F3F3F8', border: '1px solid #C3C6D4', borderRadius: 8, fontSize: 16, color: '#1A1C1F', minHeight: 120 }}
            />
          </div>
          ...
          <div className="flex items-center justify-between pt-2" style={{ borderTop: '1px solid #F1F5F9' }}>
            <div>
              <p className="text-sm font-semibold" style={{ color: '#1A1C1F' }}>Personalization Options</p>
              <p className="text-xs mt-0.5" style={{ color: '#64748B' }}>Allow customers to add custom text or engravings</p>
            </div>
            <button type="button" onClick={() => setForm(p => ({ ...p, personalization: !p.personalization }))}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="block text-sm font-semibold" style={{ color: '#1A1C1F' }}>Primary Product Image</label>
              <label className="block cursor-pointer">
                <input type="file" accept="image/*" onChange={e => handleFile(e, 'primary')} className="hidden" />
                <div className="flex flex-col items-center justify-center gap-2 p-6"
                  style={{ background: '#F8FAFC', border: '2px dashed #C3C6D4', borderRadius: 12, minHeight: 120 }}>
                  <ImagePlus size={23} color="#94A3B8" />
                  <span className="text-xs font-semibold uppercase tracking-wide" style={{ color: '#64748B' }}>Single Upload</span>
            ...
            <div className="space-y-2">
              <label className="block text-sm font-semibold" style={{ color: '#1A1C1F' }}>Product Gallery</label>
              <label className="block cursor-pointer">
                <input type="file" accept="image/*" multiple onChange={e => handleFile(e, 'gallery')} className="hidden" />
                <div className="flex flex-col items-center justify-center gap-2 p-6"
                  style={{ background: '#F8FAFC', border: '2px dashed #C3C6D4', borderRadius: 12, minHeight: 120 }}>
                  <Images size={25} color="#94A3B8" />
                  <span className="text-xs font-semibold uppercase tracking-wide" style={{ color: '#64748B' }}>Multi Upload</span>
                  <span className="text-xs" style={{ color: '#94A3B8' }}>
        ...
        {/* Footer */}
        <div className="flex-shrink-0 flex items-center justify-end gap-3 px-8 py-6" style={{ borderTop: '1px solid #F1F5F9', background: '#FAFAFA' }}>
          <button type="button" onClick={onClose}
            className="flex items-center justify-center font-bold text-base transition hover:opacity-80"
            style={{ height: 48, padding: '0 24px', background: '#E8E8ED', borderRadius: 12, color: '#434652' }}>
            Cancel
          </button>
