...
export default function ProductTable({ products, onDelete, onView, onEdit }: ProductTableProps) {
  return (
      ...
      <table className="w-full border-collapse">
        ...
        <tbody>
          {products.map((product, index) => (
            <tr key={`${product.id}-${index}`} className="transition hover:bg-slate-50/50" style={{ borderBottom: '1px solid #E8ECF4' }}>
              ...
              <td className="pl-8">
                <button type="button" onClick={() => onView(product)} className="rounded-full transition hover:opacity-80">
