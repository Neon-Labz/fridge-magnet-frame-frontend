...
export default function PersonalizationSection({
  ...
  // Only show colors if availableColors is undefined (backward compat) or has items
  // If availableColors is explicitly an empty array, don't show colors at all
  const frameColors = availableColors === undefined 
  ...
  useEffect(() => {
    ...
    const state: PersonalizationState = frameColors.length > 0 ? { option: selectedOption, frameColor: selectedFrame } : { option: selectedOption };
    onChange?.(state);
  ...
  return (
    <div className="w-full">
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        >
          <span>{options.find((o) => o.id === selectedOption)?.label ?? "Select"}</span>
          <svg xmlns="http://www.w3.org/2000/svg" className={`w-5 h-5 transition-transform ${isOpen ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
          ...
          <div className="absolute z-20 top-[calc(100%+6px)] left-0 w-full bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden">
            {options.map((opt) => (
              <button
                ...
                className={`w-full text-left px-5 py-3 ${selectedOption === opt.id ? "bg-[#F0F2F8] text-[#1A2B5E] font-semibold" : "hover:bg-gray-50"}`}
              >
                {opt.label}
              </button>
            ))}
        ...
        <div className="mt-6">
          ...
          <div className="flex items-start gap-6">
            {frameColors.map((fc) => {
              ...
              return (
                <button key={fc.id} onClick={() => setSelectedFrame(fc.id)} className="flex flex-col items-center gap-2">
                  <div className={`w-14 h-14 rounded-full flex items-center justify-center border-2 ${active ? "border-[#1A2B5E]" : "border-transparent"}`}>
                    <div className={["w-10 h-10 rounded-full transition-transform duration-200 group-hover:scale-105", fc.bg].join(" ")} />
                  </div>
                  <span className={`text-sm ${active ? "text-[#1A2B5E]" : "text-gray-400"}`}>{fc.label}</span>
                </button>
              );
            })}
          </div>
