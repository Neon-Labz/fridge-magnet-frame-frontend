...
import type { FilterStatus, SortBy } from '@/components/dashboard/products/ProductFilters';
...
export default function ProductsPage() {
  ...
  const handleFilterSelect = (f: FilterStatus) => { setFilterStatus(f); setPage(1); setFilterOpen(false); };
  const handleSortSelect = (s: SortBy) => { setSortBy(s); setPage(1); setSortOpen(false); };
  ...
  const handleUpdateProduct = (product: Product, newStatus: string) => {
    ...
    setProducts(prev => prev.map(p => 
      p.id === product.id 
        ? { ...p, stockStatus: statusMap[newStatus] || p.stockStatus }
        : p
    ));
  };
  ...
  return (
    <>
      ...
      <ViewProductModal isOpen={!!viewTarget} product={viewTarget} onClose={() => setViewTarget(null)} onUpdate={handleUpdateProduct} />
